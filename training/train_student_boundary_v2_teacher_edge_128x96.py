...
OUTPUT_CHECKPOINT = os.path.join(
    CHECKPOINT_DIR,
    "student_boundary_v2_teacher_edge_128x96_best.pth"
)
...
# V2 keeps the V1 loss recipe and adds teacher-gradient guidance.
LAMBDA_GT = 1.0
LAMBDA_TEACHER = 1.0
LAMBDA_GRADIENT = 0.25
LAMBDA_TEACHER_EDGE = 0.10
...
def compute_loss(
    ...
    V1 losses are preserved:
      1. GT depth loss
      2. Teacher depth loss
      3. GT gradient loss

    V2 adds:
      4. Teacher gradient loss

    The teacher gradient is computed directly from the existing
    cached teacher depth map. No new dataset is required.
    """
    ...
    gt_edge_loss = gradient_loss(
        prediction,
        gt,
        mask
    )

    teacher_edge_loss = gradient_loss(
        prediction,
        teacher,
        mask
    )
    ...
    total_loss = (
        ...
        + LAMBDA_GRADIENT * gt_edge_loss
        + LAMBDA_TEACHER_EDGE * teacher_edge_loss
    )
...
def train_one_epoch():
    ...
    for batch in train_loader:
        ...
        total_teacher_edge += teacher_edge_loss.item()
        batches += 1
...
print("\n" + "=" * 70)
print("STARTING BOUNDARY-PRESERVING V2 — TEACHER EDGE TRAINING")
print("=" * 70)
